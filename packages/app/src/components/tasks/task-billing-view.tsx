...
// Remount key for the three text inputs. AdaptiveTextInput is native-owned: it
// DROPS a `value` prop and only seeds from `initialValue` at mount, so the
// fields would render blank forever on an analyzed task without this. The key
// tracks the resolved seed (not the drafts), so it changes when a fresh estimate
// lands or the user switches task — and never while typing.
function billingSeedKey(taskId: string, seed: ResolvedBilling): string {
  return [taskId, seed.billingTitle, seed.billingDescription, seed.billingHours ?? ""].join("|");
}
...
export function TaskBillingView({
  ...
  const seedHours = seed.billingHours !== undefined ? String(seed.billingHours) : "";
  ...
  const [hoursDraft, setHoursDraft] = useState(seedHours);
  // Changes only when the resolved seed does, so the inputs re-seed on a fresh
  // estimate (or another task) and stay untouched while the user types.
  const seedKey = billingSeedKey(task.id, seed);
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
      <View style={styles.card}>
        ...
        <Field label={t("tasks.panel.billingLine.description")}>
          <FormTextInput
            initialValue={seed.billingDescription}
            resetKey={seedKey}
