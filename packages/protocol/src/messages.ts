...
import {
  ...
  TasksTaskRetryAnalysisRequestSchema,
  TasksTaskApproveRequestSchema,
  TasksTaskValidateRequestSchema,
  TasksTaskDeployRequestSchema,
  TasksTaskGitRefreshRequestSchema,
  TasksTaskGitResumeConflictRequestSchema,
  TasksBoardDeployAllRequestSchema,
  TasksTaskArchiveRequestSchema,
  TasksProposalResolveRequestSchema,
  ...
  TasksTaskMarkViewedResponseSchema,
  TasksTaskDeleteResponseSchema,
  TasksTaskEstimateResponseSchema,
  TasksTaskRunNowResponseSchema,
  TasksTaskRetryAnalysisResponseSchema,
  TasksTaskApproveResponseSchema,
  TasksTaskValidateResponseSchema,
  TasksTaskDeployResponseSchema,
  TasksTaskGitRefreshResponseSchema,
  TasksTaskGitResumeConflictResponseSchema,
  TasksBoardDeployAllResponseSchema,
  TasksTaskArchiveResponseSchema,
  TasksProposalResolveResponseSchema,
  TasksConductorEnsureResponseSchema,
  TasksBoardUpdateMessageSchema,
} from "./tasks/rpc-schemas.js";
import { TaskChatProposalSchema } from "./tasks/types.js";
import {
...
const MutableBrainMemoryConfigSchema = z
  .object({
    enabled: z.boolean().default(false),
  })
  .passthrough();
export const MutableDaemonConfigSchema = z
  .object({
    ...
    brainMemory: MutableBrainMemoryConfigSchema.default({ enabled: false }),
...
export const MutableDaemonConfigPatchSchema = z
  .object({
    ...
    brainMemory: MutableBrainMemoryConfigSchema.partial().optional(),
    providers: z
...
export const AgentTimelineItemPayloadSchema: z.ZodType<AgentTimelineItem, unknown> = z.union([
  ...
  z.object({
    ...
    // cards (approve/refuse/edit) WITHOUT anything being written to the board:
    // the task is created only on approval. Additive: old items without it fall
    // back to the count-only pill; legacy items carried only { taskId, title }.
    tasks: z.array(TaskChatProposalSchema).optional(),
  }),
...
export const CreateAgentRequestMessageSchema = z.object({
  ...
  env: z.record(z.string(), z.string()).optional(),
  workspaceId: z.string().optional(),
  // Restauré : contexte de l'appelant, pour qu'une création lancée en ligne de
  // commande hérite du même espace de travail et de la même parenté qu'une
  // création faite par un agent.
...
// Restauré : gestion des scripts d'un espace de travail (lister, démarrer,
// arrêter). Le service serveur correspondant existe déjà ; seuls les messages et
// leur routage manquaient, ce qui rendait « paseo script » inopérant.
...
const PullRequestTimelineCommentItemSchema = z.object({
  ...
  // Restauré : identifiant de fil indépendant d'une position dans un fichier,
  // pour que les réponses générales (celles de GitLab notamment) se regroupent
  // en une seule discussion.
  threadId: z.string().optional(),
  // Restauré : état de résolution d'un fil sans position de fichier. Les fils
  // rattachés à une ligne portent le leur sous `location.isResolved`.
